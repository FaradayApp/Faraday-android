...
//    }
    ...
    fun getPlaceholderDrawable(matrixItem: MatrixItem, userInRoomInformation: MatrixItemColorProvider.UserInRoomInformation? = null): Drawable =
            AvatarHelper.getPlaceholderDrawable(matrixItemColorProvider, dimensionConverter, matrixItem, userInRoomInformation)
    ...
    private fun resolvedUrl(avatarUrl: String?): String? {
        return activeSessionHolder.getSafeActiveSession()?.contentUrlResolver()?.let {
            AvatarHelper.resolvedUrl(it, avatarUrl)
        }
    }
    ...
    private fun ImageView.setContentDescription(matrixItem: MatrixItem) {
        ...
        AvatarHelper.getContentDescription(stringProvider, matrixItem)?.also {
